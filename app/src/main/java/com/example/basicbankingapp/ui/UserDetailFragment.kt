    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
           ...
           // ViewCompat.setTransitionName(binding.detailScreen, "title_${args.user.userId}")
            ...
            requireActivity().findViewById<ImageView>(R.id.profile_image_toolbar).setImageResource(args.user.userProfilePicture)
        ...
        sharedViewModel.listTransaction(args.user.userId).observe(viewLifecycleOwner) { list ->
            ...
            if (it.isNotEmpty()) {
                ...
                binding.userTransaction1.userTransfer.text =
                    it[0].transferAmount.formatMoneyAmount().formatMoneyTransaction(it[0].transactionType)
                binding.userTransaction1.userImageDetailTransaction.setImageResource(it[0].transferToProfile)
                binding.userTransaction1.userTransferTime.text = it[0].transferDate.formatDate()
                binding.userTransaction1.userTransfer.setTextColor(formatSendReceiveColor
                    (transactionType = it[0].transactionType, context = requireContext()))
                ...
                if (it.size >= 2) {
                    ...
                    binding.userTransaction2.userTransfer.text =
                        it[1].transferAmount.formatMoneyAmount().formatMoneyTransaction(it[1].transactionType)
                    binding.userTransaction2.userImageDetailTransaction.setImageResource(it[1].transferToProfile)
                    binding.userTransaction2.userTransferTime.text = it[1].transferDate.formatDate()
                    binding.userTransaction2.userTransfer.setTextColor(
                        formatSendReceiveColor(transactionType = it[1].transactionType, context = requireContext()))
                    ...
                    if (it.size >= 3) {
                        ...
                        binding.userTransaction3.userTransfer.text =
                            it[2].transferAmount.formatMoneyAmount().formatMoneyTransaction(it[2].transactionType)
                        ...
                        binding.userTransaction3.userTransferTime.text =
                            it[2].transferDate.formatDate()
                        binding.userTransaction3.userTransfer.setTextColor(
                            formatSendReceiveColor(transactionType = it[2].transactionType, context = requireContext()))
